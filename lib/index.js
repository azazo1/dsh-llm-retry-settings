...
// ../../AppData/Local/DSH Desktop Rust/resources/app/node_modules/@deepseek-ai/cosmokit/lib/index.js
...
// ../../AppData/Local/DSH Desktop Rust/resources/app/node_modules/@deepseek-ai/schemastery/lib/index.mjs
...
var z = (() => {
  try {
    ...
    const live = req("@deepseek-ai/schemastery");
...
var vol = (f) => typeof f.volatile === "function" ? f.volatile() : f;
var VOLATILE_WRITE = /* @__PURE__ */ Symbol.for("cosmokit.volatile.write");
var unvol = (v) => v && typeof v === "object" && VOLATILE_WRITE in v && typeof v.get === "function" ? v.get() : v;
var DIAG_TAG = "v0.1.11";
...
var summaryOf = (cfg) => `enabled=${cfg.enabled} codes=${cfg.retryableCodes.length} maxRetries=${cfg.maxRetries} backoff=${cfg.initialDelayMs}~${cfg.maxDelayMs}ms jitter=${cfg.jitterRatio} autoContinue=${cfg.autoContinue} maxContinuations=${cfg.maxContinuations} continueOnError=${cfg.continueOnError} overrides=${cfg.overrides.length}`;
var STATS_PATH = "/dsh-llm-retry-settings/stats";
var LOG_ROUTE_PATH = "/dsh-llm-retry-settings/log";
var RECENT_MAX = 50;
var STATE_MAX = 200;
var stats = {
  startedAt: Date.now(),
...
var inboxBusy = (agent) => {
  ...
  const size = (arr) => Array.isArray(arr) ? arr.length : 0;
...
var matchOverride = (rows, provider, model) => rows.find((row) => {
  if (!row) return false;
...
var normOverrides = (raw, fallback) => {
...
var asBool = (v) => {
  const x = unvol(v);
  return typeof x === "boolean" ? x : void 0;
};
var asInt = (v, min) => {
  const x = unvol(v);
  return typeof x === "number" && Number.isFinite(x) ? Math.max(min, Math.floor(x)) : void 0;
};
var asFloat = (v, min, max) => {
  const x = unvol(v);
  return typeof x === "number" && Number.isFinite(x) ? Math.min(max, Math.max(min, x)) : void 0;
};
var DEFAULTS_CONFIG = { ...DEFAULTS, retryableCodes: [...DEFAULTS.retryableCodes] };
...
function apply(ctx, config) {
  ...
  ctx.on(
    ...
    (payload, next) => {
      ...
      diag(
        `request-error enabled=${cfg.enabled} code=${code || "(n/a)"} provider=${provider || "(n/a)"} model=${model || "(n/a)"}${override ? ` override=${override.provider}/${override.model}` : ""}`
      );
      if (cfg.enabled) {
        stats.retries += 1;
  ...
  const handleTurnEnd = (session, turn, kind, via, agentHint, errorCode) => {
    const cfg = current();
    ...
    diag(
      `turn/end via=${via} session=${session.id} turn=${turn} kind=${kind}${transientFailure ? ` code=${errorCode}` : ""} autoContinue=${cfg.autoContinue} maxContinuations=${cfg.maxContinuations} chain=${state.chain}`
    );
